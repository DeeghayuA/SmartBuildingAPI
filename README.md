...
**A:** JAX-RS filters handle **Cross-Cutting Concerns** in a centralized location. Using filters ensures that *every* request and response is logged automatically without needing to repeat code in every method. This follows the **DRY (Don't Repeat Yourself)** principle and makes the logging behavior much easier to modify, enable, or disable globally across the entire application.
